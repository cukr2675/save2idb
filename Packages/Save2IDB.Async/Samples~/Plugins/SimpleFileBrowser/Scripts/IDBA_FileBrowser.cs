...
using Save2IDB.Async;

namespace SimpleFileBrowser
{
	public class IDBA_FileBrowser : MonoBehaviour, IListViewAdapter
	{
		[SerializeField] private IDBA_CoroutineRunner _IDBA_CoroutineObj = null;
...
#pragma warning restore 0649
		...
		private static FileSystemEntryFilter m_displayedEntriesFilter = default;
			...
			set
			{
				if( m_pickFolderQuickLinkText != value )
				{
					...
					if( m_instance )
					{
						...
						{
							IDBA_FileBrowserQuickLink quickLink = m_instance.allQuickLinks[i];
		...
		private static IDBA_FileBrowser m_instance = null;
		private static IDBA_FileBrowser Instance
		{
			get
			{
				if( !m_instance )
				{
					m_instance = Instantiate( Resources.Load<GameObject>("IDBA_SimpleFileBrowserCanvas") ).GetComponent<IDBA_FileBrowser>();
...
#pragma warning restore 0414
		...
		[SerializeField]
		private IDBA_FileBrowserMovement window;
		...
		[SerializeField]
		private IDBA_FileBrowserItem itemPrefab;
		private readonly List<IDBA_FileBrowserItem> allItems = new List<IDBA_FileBrowserItem>( 16 );

		[SerializeField]
		private IDBA_FileBrowserQuickLink quickLinkPrefab;
		private readonly List<IDBA_FileBrowserQuickLink> allQuickLinks = new List<IDBA_FileBrowserQuickLink>( 8 );
		...
		[SerializeField]
		private IDBA_RecycledListView listView;
		...
		[SerializeField]
		private IDBA_FileBrowserRenamedItem renameItem;

		[SerializeField]
		private IDBA_FileBrowserContextMenu contextMenu;

		[SerializeField]
		private IDBA_FileBrowserFileOperationConfirmationPanel fileOperationConfirmationPanel;

		[SerializeField]
		private IDBA_FileBrowserAccessRestrictedPanel accessRestrictedPanel;

		[SerializeField]
		private IDBA_FileBrowserCursorHandler resizeCursorHandler;
#pragma warning restore 0649
...
#pragma warning restore 0414
		...
		private string CurrentPath
		{
			...
			//set
			private async UniTask SetCurrentPath(string value)
			{
				if( value != null )
				{
					value = value.Trim();
				...
				{
					if( !await IDBA_FileBrowserHelpers.DirectoryExists( value ) )
					{
						pathInputField.text = m_currentPath;
						return;
					}
		...
		private string SearchString
		{
			...
			//set
			private async UniTask SetSearchString(string value)
			{
		...
		private void Awake()
		{
			_IDBA_CoroutineObj = new GameObject().AddComponent<IDBA_CoroutineRunner>();
			...
			backButton.onClick.AddListener(Task2Action(OnBackButtonClicked));
			forwardButton.onClick.AddListener(Task2Action(OnForwardButtonClicked));
			upButton.onClick.AddListener(Task2Action(OnUpButtonClicked));
			moreOptionsButton.onClick.AddListener( OnMoreOptionsButtonClicked );
			submitButton.onClick.AddListener( OnSubmitButtonClicked );
			cancelButton.onClick.AddListener( OnCancelButtonClicked );
			pathInputField.onEndEdit.AddListener(Task2Action<string>(OnPathChanged));
			searchInputField.onValueChanged.AddListener(Task2Action<string>(OnSearchStringChanged));
			...
			filtersDropdown.onValueChanged.AddListener(Task2Action<int>(OnFilterChanged));
			showHiddenFilesToggle.onValueChanged.AddListener(Task2Action<bool>(OnShowHiddenFilesToggleChanged));
...
//#endif
		...
		private async UniTask OnApplicationFocus( bool focus )
		{
			if( !focus )
		...
		IDBA_ListItem IListViewAdapter.CreateItem()
		{
			IDBA_FileBrowserItem item = (IDBA_FileBrowserItem) Instantiate( itemPrefab, filesContainer, false );
		...
		void IListViewAdapter.SetItemContent( IDBA_ListItem item )
		{
			IDBA_FileBrowserItem file = (IDBA_FileBrowserItem) item;
		...
		private async UniTask InitializeQuickLinks()
		{
			...
			for( int i = 0; i < quickLinks.Length; i++ )
			{
				...
				await AddQuickLink( quickLink.icon, quickLink.name, quickLinkPath );
			}
		...
		private async UniTask RefreshDriveQuickLinks()
		{
			...
			IDBA_FileBrowserQuickLink[] customQuickLinks = allQuickLinks.Count > 0 ? allQuickLinks.ToArray() : null;
			allQuickLinks.Clear();
...
#else
				if( await AddQuickLink( m_skin.DriveIcon, drives[i], drives[i] ) )
					numberOfDriveQuickLinks++;
#endif
			}
#endif
			...
			try
			{
				if( !string.IsNullOrEmpty( m_currentPath ) && !await IDBA_FileBrowserHelpers.DirectoryExists( m_currentPath ) )
				{
					string currentPathRoot = Path.GetPathRoot( m_currentPath );
					if( !string.IsNullOrEmpty( currentPathRoot ) && await IDBA_FileBrowserHelpers.DirectoryExists( currentPathRoot ) )
						await SetCurrentPath(currentPathRoot);
					else if( allQuickLinks.Count > 0 )
						await SetCurrentPath(allQuickLinks[0].TargetPath);
				}
			}
			catch { }
		}
		...
		private async UniTask OnBackButtonClicked()
		{
			if( currentPathIndex > 0 )
				await SetCurrentPath(pathsFollowed[--currentPathIndex]);
		}

		private async UniTask OnForwardButtonClicked()
		{
			if( currentPathIndex < pathsFollowed.Count - 1 )
				await SetCurrentPath(pathsFollowed[++currentPathIndex]);
		}

		private async UniTask OnUpButtonClicked()
		{
#if !UNITY_EDITOR && UNITY_ANDROID
			if( FileBrowserHelpers.ShouldUseSAF )
			{
...
#endif
			{
				try // When "C:/" or "C:" is typed instead of "C:\", an exception is thrown
				{
					...
					if (parentPath != null)
						await SetCurrentPath(parentPath.FullName);
				}
				catch
				{
				}
		...
		private void OnSubmitButtonClicked() => Task2Action(OnSubmitButtonClickedAsync)();
		private async UniTask OnSubmitButtonClickedAsync()
		{
			string[] result = null;
				...
				if( m_allowMultiSelection && selectedFileEntries.Count > 1 )
				{
					...
					if( m_pickerMode == PickMode.Files )
					{
						for( int i = 0; i < selectedFileEntries.Count; i++ )
						{
							...
							{
								await SetCurrentPath(validFileEntries[selectedFileEntries[i]].Path);
								return;
							}
						}
					}
				...
				else
				{
					...
					for( int startIndex = 0, nextStartIndex = 0; startIndex < filenameInput.Length; startIndex = nextStartIndex )
					{
						...
						if( !VerifyFilename( filename ) )
						{
							...
							try
							{
								if( await IDBA_FileBrowserHelpers.DirectoryExists( filename ) )
								{
									FileSystemEntry fileEntry = new FileSystemEntry( filename, IDBA_FileBrowserHelpers.GetFilename( filename ), "", true );
									if( FileSystemEntryMatchesFilters( fileEntry, AllExtensionsHaveSingleSuffix ) )
									{
										if( m_pickerMode == PickMode.Files )
										{
											await SetCurrentPath(filename);
											return;
										}
										else
										{
								...
								else if( m_pickerMode != PickMode.Folders && await IDBA_FileBrowserHelpers.FileExists( filename ) )
								{
									string fullPathFilename = IDBA_FileBrowserHelpers.GetFilename( filename );
									...
									if( FileSystemEntryMatchesFilters( fileEntry, AllExtensionsHaveSingleSuffix ) )
									{
										...
										if( m_acceptNonExistingFilename )
											submittedFolderPaths.Add( IDBA_FileBrowserHelpers.GetDirectoryName( filename ) );
						...
						try
						{
							...
							if( fileEntryIndex >= 0 ) // This is an existing file/folder
							{
								if( validFileEntries[fileEntryIndex].IsDirectory && m_pickerMode == PickMode.Files )
								{
									...
									await SetCurrentPath(validFileEntries[fileEntryIndex].Path);
									return;
								}
								else
								{
			...
			if( result != null )
			{
				if( m_checkWriteAccessToDestinationDirectory )
				{
					...
					{
						if( !string.IsNullOrEmpty( submittedFolderPaths[i] ) && !await CheckDirectoryWriteAccess( submittedFolderPaths[i] ) )
						{
				...
				{
					fileOperationConfirmationPanel.Show( this, submittedFileEntriesToOverwrite, IDBA_FileBrowserFileOperationConfirmationPanel.OperationType.Overwrite, () => OnOperationSuccessful( result ) );
					return;
				}
		...
		private async UniTask OnPathChanged( string newPath )
		{
		...
		private async UniTask OnSearchStringChanged( string newSearchString )
		{
			if( !canvas ) // Same as OnPathChanged
				return;

			PersistFileEntrySelection();
			await SetSearchString(newSearchString);
		}

		private async UniTask OnFilterChanged( int value )
		{
			if( !canvas ) // Same as OnPathChanged
				return;
			...
			PersistFileEntrySelection();
			await RefreshFiles( extensionsSingleSuffixModeChanged );
		}

		private async UniTask OnShowHiddenFilesToggleChanged( bool value )
		{
			if( !canvas ) // Same as OnPathChanged
				return;
		...
		public async UniTask OnItemSelected( IDBA_FileBrowserItem item, bool isDoubleClick )
		{
			if( item == null )
				return;

			if( item is IDBA_FileBrowserQuickLink )
			{
#if !UNITY_EDITOR && UNITY_ANDROID
...
#endif
				await SetCurrentPath(( (IDBA_FileBrowserQuickLink) item ).TargetPath);
...
#endif
					await SetCurrentPath(Path.Combine( m_currentPath, item.Name ));
				}
			}
		}

		public async UniTask OnItemHeld( IDBA_FileBrowserItem item )
		{
			if( item is IDBA_FileBrowserQuickLink )
				await OnItemSelected( item, false );
		...
		public async UniTask Show( string initialPath, string initialFilename )
		{
			if( AskPermissions )
				RequestPermission();

			if( !quickLinksInitialized )
				await InitializeQuickLinks();
			...
			await SetCurrentPath(await GetInitialPath( initialPath ));
		...
		public async UniTask RefreshFiles( bool pathChanged )
		{
			...
			{
				if( !string.IsNullOrEmpty( m_currentPath ) )
					allFileEntries = await IDBA_FileBrowserHelpers.GetEntriesInDirectory( m_currentPath, allExtensionsHaveSingleSuffix );
				else
					allFileEntries = null;
			}
		...
		{
			_IDBA_CoroutineObj.StartCoroutine( CreateNewFolderCoroutine() );
		}
		...
		private IEnumerator CreateNewFolderCoroutine()
		{
			...
			{
				_IDBA_CoroutineObj.StartCoroutine(Coroutine(folderName));
			} );

			IEnumerator Coroutine(string folderName)
			{
				...
				yield return IDBA_FileBrowserHelpers.CreateFolderInDirectory(CurrentPath, folderName).ToCoroutine();

				pendingFileEntrySelection.Clear();
				pendingFileEntrySelection.Add(folderName);

				yield return RefreshFiles(true).ToCoroutine();
		...
		public void RenameSelectedFile()
		{
			...
			{
				_IDBA_CoroutineObj.StartCoroutine(Coroutine(newName));
			} );

			IEnumerator Coroutine(string newName)
			{
				if (string.IsNullOrEmpty(newName) || newName == fileInfo.Name)
					yield break;

				if (fileInfo.IsDirectory)
					yield return IDBA_FileBrowserHelpers.RenameDirectory(fileInfo.Path, newName).ToCoroutine();
				else
					yield return IDBA_FileBrowserHelpers.RenameFile(fileInfo.Path, newName).ToCoroutine();
		...
		public void DeleteSelectedFiles()
		{
			...
			fileOperationConfirmationPanel.Show( this, validFileEntries, selectedFileEntries, IDBA_FileBrowserFileOperationConfirmationPanel.OperationType.Delete, () =>
			{
				_IDBA_CoroutineObj.StartCoroutine(Coroutine());
			} );
			...
			IEnumerator Coroutine()
			{
				for (int i = selectedFileEntries.Count - 1; i >= 0; i--)
				{
					...
					if (fileInfo.IsDirectory)
						yield return IDBA_FileBrowserHelpers.DeleteDirectory(fileInfo.Path).ToCoroutine();
					else
						yield return IDBA_FileBrowserHelpers.DeleteFile(fileInfo.Path).ToCoroutine();
				}
		...
		private async UniTask<bool> AddQuickLink( Sprite icon, string name, string path )
		{
			...
			{
#if !WIN_DIR_CHECK_WITHOUT_TIMEOUT && ( UNITY_EDITOR_WIN || UNITY_STANDALONE_WIN )
				if( !await CheckDirectoryExistsWithTimeout( path ) )
#else
				//if( !Directory.Exists( path ) )
				if( !await IDBFileTask.ExistsAsync( path ) )
#endif
					return false;
			...
			IDBA_FileBrowserQuickLink quickLink = (IDBA_FileBrowserQuickLink) Instantiate( quickLinkPrefab, quickLinksContainer, false );
		...
		private async UniTask<string> GetInitialPath( string initialPath )
		{
			if( !string.IsNullOrEmpty( initialPath ) && !await IDBA_FileBrowserHelpers.DirectoryExists( initialPath ) && await IDBA_FileBrowserHelpers.FileExists( initialPath ) )
			{
				// Path points to a file, use its parent directory's path instead
				initialPath = IDBA_FileBrowserHelpers.GetDirectoryName( initialPath );
			}

			if( string.IsNullOrEmpty( initialPath ) || !await IDBA_FileBrowserHelpers.DirectoryExists( initialPath ) )
			{
				...
				else
				{
					...
					if( !string.IsNullOrEmpty( lastBrowsedFolder ) && await IDBA_FileBrowserHelpers.DirectoryExists( lastBrowsedFolder ) )
...
#if !WIN_DIR_CHECK_WITHOUT_TIMEOUT && ( UNITY_EDITOR_WIN || UNITY_STANDALONE_WIN )
		private async UniTask<bool> CheckDirectoryExistsWithTimeout( string path, int timeout = 750 )
		{
			...
			catch
			{
				...
				directoryExists = await IDBFileTask.ExistsAsync(path);
			}
		...
		private async UniTask<bool> CheckDirectoryWriteAccess( string path )
		{
#if !UNITY_EDITOR && UNITY_ANDROID
			if( FileBrowserHelpers.ShouldUseSAFForPath( path ) )
				return true;
			...
			try
			{
				...
				await(await IDBFileTask.OpenAsync(tempFilePath, FileMode.Create)).DisposeAsync();
				//File.Delete( tempFilePath );
				await IDBFileTask.DeleteAsync(tempFilePath);
		...
		public static async UniTask<bool> ShowSaveDialog( OnSuccess onSuccess, OnCancel onCancel,
		...
		{
			return await ShowDialogInternal( onSuccess, onCancel, pickMode, allowMultiSelection, pickMode != PickMode.Folders, initialPath, initialFilename, title, saveButtonText );
		}

		public static async UniTask<bool> ShowLoadDialog( OnSuccess onSuccess, OnCancel onCancel,
										   PickMode pickMode, bool allowMultiSelection = false,
										   string initialPath = null, string initialFilename = null,
										   string title = "Load", string loadButtonText = "Select" )
		{
			return await ShowDialogInternal( onSuccess, onCancel, pickMode, allowMultiSelection, false, initialPath, initialFilename, title, loadButtonText );
		}

		private static async UniTask<bool> ShowDialogInternal( OnSuccess onSuccess, OnCancel onCancel,
												...
												string initialPath, string initialFilename, string title, string submitButtonText )
		{
			...
			await Instance.Show( initialPath, initialFilename );
		...
		public static IEnumerator WaitForSaveDialog( PickMode pickMode, bool allowMultiSelection = false,
													 ...
													 string title = "Save", string saveButtonText = "Save" )
		{
			var task = ShowSaveDialog(null, null, pickMode, allowMultiSelection, initialPath, initialFilename, title, saveButtonText);
			var result = false;
			yield return task.ToCoroutine(value =>
			{
				result = value;
			});
			if (!result)
		...
		public static IEnumerator WaitForLoadDialog( PickMode pickMode, bool allowMultiSelection = false,
													 ...
													 string title = "Load", string loadButtonText = "Select" )
		{
			var task = ShowSaveDialog(null, null, pickMode, allowMultiSelection, initialPath, initialFilename, title, loadButtonText);
			var result = false;
			yield return task.ToCoroutine(value =>
			{
				result = value;
		...
		public static async UniTask<bool> AddQuickLink( string name, string path, Sprite icon = null )
		{
			if( string.IsNullOrEmpty( path ) || !await IDBA_FileBrowserHelpers.DirectoryExists( path ) )
				return false;
			...
			if( !quickLinksInitialized )
			{
				...
				await Instance.InitializeQuickLinks();
			}

			return await Instance.AddQuickLink( icon, name, path );
		}
		...
		private UnityEngine.Events.UnityAction Task2Action(Func<UniTask> task)
		{
			return () => _IDBA_CoroutineObj.StartCoroutine(task().ToCoroutine());
		}

		private UnityEngine.Events.UnityAction<T> Task2Action<T>(Func<T, UniTask> action)
		{
			return x => _IDBA_CoroutineObj.StartCoroutine(action(x).ToCoroutine());
		}
	}
}