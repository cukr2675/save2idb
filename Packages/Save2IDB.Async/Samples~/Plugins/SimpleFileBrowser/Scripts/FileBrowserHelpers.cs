...
namespace SimpleFileBrowser
{
	public struct FileSystemEntry
	{
		...
		/// <param name="a">for IDBA</param>
		public FileSystemEntry( FileSystemInfo fileInfo, string extension, FileAttributes a )
		{
	...
	public static class IDBA_FileBrowserHelpers
	{
		...
		public static async UniTask<bool> FileExists( string path )
		{
#if !UNITY_EDITOR && UNITY_ANDROID
			...
			return await IDBFileTask.ExistsAsync(path);
		}

		public static async UniTask<bool> DirectoryExists( string path )
		{
#if !UNITY_EDITOR && UNITY_ANDROID
			if( ShouldUseSAFForPath( path ) )
...
//		}
		...
		public static async UniTask<FileSystemEntry[]> GetEntriesInDirectory( string path, bool extractOnlyLastSuffixFromExtensions )
		{
#if !UNITY_EDITOR && UNITY_ANDROID
			if( ShouldUseSAFForPath( path ) )
			{
			...
			try
			{
				...
				string[] files = await IDBDirectoryTask.GetFilesAsync(path);
				//string[] subDirectories = Directory.GetDirectories( path );
				string[] subDirectories = await IDBDirectoryTask.GetDirectoriesAsync(path);
				...
				for( int i = 0; i < files.Length; i++ )
				{
					try
					{
						...
						result[index] = new FileSystemEntry( fileInfo, IDBA_FileBrowser.GetExtensionFromFilename( fileInfo.Name, extractOnlyLastSuffixFromExtensions ), default);
						index++;
					}
				...
				for ( int i = 0; i < subDirectories.Length; i++ )
				{
					try
					{
						result[index] = new FileSystemEntry(new DirectoryInfo(subDirectories[i]), string.Empty, FileAttributes.Directory);
						index++;
					}
					catch( System.Exception e )
					{
...
//		}
		...
		public static async UniTask<string> CreateFolderInDirectory( string directoryPath, string folderName )
		{
#if !UNITY_EDITOR && UNITY_ANDROID
			if( ShouldUseSAFForPath( directoryPath ) )
			...
			await IDBDirectoryTask.CreateDirectoryAsync(path);
			return path;
		}
...
//		}
		...
		public static async UniTask<string> RenameFile( string path, string newName )
		{
#if !UNITY_EDITOR && UNITY_ANDROID
			if( ShouldUseSAFForPath( path ) )
			...
			await IDBFileTask.MoveAsync(path, newPath);

			return newPath;
		}

		public static async UniTask<string> RenameDirectory( string path, string newName )
		{
#if !UNITY_EDITOR && UNITY_ANDROID
			if( ShouldUseSAFForPath( path ) )
				return AJC.CallStatic<string>( "RenameSAFEntry", Context, path, newName );
		...
		public static async UniTask DeleteFile( string path )
		{
#if !UNITY_EDITOR && UNITY_ANDROID
			if( ShouldUseSAFForPath( path ) )
			{
...
#endif
			//File.Delete( path );
			await IDBFileTask.DeleteAsync(path);
		}

		public static async UniTask DeleteDirectory( string path )
		{
#if !UNITY_EDITOR && UNITY_ANDROID
			if( ShouldUseSAFForPath( path ) )
			{
			...
			await IDBDirectoryTask.DeleteAsync(path);
		}
